...
class Color {
    ...
    rgbToHex(rgb) {
        return rgb2hex_1.default(rgb);
    }
    hexToRgb(hex) {
        return hex_1.default(hex).map((val) => Math.round(val));
    }
    rgbToHsv(rgb) {
        return rgb2hsv_1.default(rgb).map((val) => Math.round(val));
    }
    hsvToRgb(hsv) {
        return hsv2rgb_1.default(hsv).map((val) => Math.round(val));
    }
}
